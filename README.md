...
Apple Music M4A downloads can use **gamdl 3.9.1 or newer** when it is installed separately. BlindDL finds `gamdl` on PATH, the `BLINDDL_GAMDL` environment variable, and common pip/venv locations. Gamdl runs as its own process so its PlayReady dependency stack does not force BlindDL to downgrade its newer cryptography libraries. If gamdl is not available, BlindDL keeps using its built-in Apple Music downloader; MP3 output always uses BlindDL's built-in path because gamdl does not produce BlindDL's MP3 presets.
...
Closing the window and minimizing it both put BlindDL in the system tray. The high-contrast blue **B** icon shows a notification when the window hides; click it once, press the global hotkey (Ctrl+Alt+B unless changed in Settings, Interface, Window behavior), or launch BlindDL again to restore the existing window. (Windows+B used to be suggested here, but on Windows 11 that combination belongs to the system and can never reach an application.) Windows may place new notification icons in its tray-overflow menu until you pin them. BlindDL never hides if Windows has not confirmed that its tray icon was installed. Queued downloads, uploads, seeding torrents, chat, and subscription checks keep running while it is there. Closing means every way Windows closes a window, including Alt+F4 and the system menu. File, Exit and Ctrl+Q always exit for real, as does Exit on the tray menu, and the two hide behaviours can be switched off in Settings, Window.
