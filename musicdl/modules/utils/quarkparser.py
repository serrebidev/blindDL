...
class QuarkParser():
    ...
    def _parsefromdirurl(url: str, passcode: str = '', cookies: str | dict = ''):
        # init
        session, download_result, cookies, pwd_id = requests.Session(), {}, cookies2dict(cookies), urlparse(url).path.strip('/').split('/')[-1]
        headers = {'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.71 Safari/537.36 Core/1.94.225.400 QQBrowser/12.2.5544.400', 'origin': 'https://pan.quark.cn', 'referer': 'https://pan.quark.cn/', 'accept-language': 'zh-CN,zh;q=0.9'}
        ...
        (resp := session.post('https://drive-pc.quark.cn/1/clouddrive/file/info/path_list', params=params, json={"file_path": ["/来自：分享"]}, cookies=cookies, headers=headers)).raise_for_status()
        ...
        params = {
            'pr': 'ucpro', 'fr': 'pc', 'uc_param_str': '', 'ver': '2', 'pwd_id': pwd_id, 'stoken': stoken, 'pdir_fid': pdir_fid, 'force': '0', '_page': '1', '_size': '50', '_fetch_banner': '0',
            '_fetch_share': '0', 'fetch_relate_conversation': '0', '_fetch_total': '1', '_sort': 'file_type:asc,file_name:asc', '__dt': '1804336', '__t': f'{int(time.time() * 1000)}',
        }
        (resp := session.get('https://drive-h.quark.cn/1/clouddrive/share/sharepage/detail', params=params, cookies=cookies, headers=headers)).raise_for_status()
        ...
        json_data = {'pwd_id': pwd_id, 'stoken': stoken, 'pdir_fid': pdir_fid, 'to_pdir_fid': to_pdir_fid, 'fid_list': [file_list[0]['fid']], 'fid_token_list': [file_list[0]['share_fid_token']], 'scene': 'link'}
        ...
        for retry_index in range(5):
            try:
                ...
                task_data = resp2json(resp=resp); fid_encrypt = task_data['data']['save_as']['save_as_top_fids'][0]; download_result['task_data'] = task_data; break
            except Exception:
        ...
        (resp := session.post('https://drive-pc.quark.cn/1/clouddrive/file/download', params=params, json={'fids': [fid_encrypt]}, cookies=cookies, headers=headers)).raise_for_status()
    ...
    def _parsefromurl(url: str, passcode: str = '', cookies: str | dict = ''):
        ...
        (resp := session.post('https://drive-h.quark.cn/1/clouddrive/share/sharepage/token', params=params, json={'pwd_id': pwd_id, 'passcode': passcode}, cookies=cookies, headers=headers)).raise_for_status()
        token_data = resp2json(resp=resp); stoken = token_data['data']['stoken']; download_result['token_data'] = token_data; time.sleep(0.1)
        # share/sharepage/detail
        params = {
            "pr": "ucpro", "fr": "pc", "uc_param_str": "", "ver": "2", "pwd_id": pwd_id, "stoken": stoken, "pdir_fid": "0", "force": "0", "_page": "1", "_size": "50", "_fetch_banner": "1",
            "_fetch_share": "1", "fetch_relate_conversation": "1", "_fetch_total": "1", "_sort": "file_type:asc,file_name:asc", "__dt": "1020", "__t": f"{int(time.time() * 1000)}"
        }
        (resp := session.get('https://drive-h.quark.cn/1/clouddrive/share/sharepage/detail', params=params, cookies=cookies, headers=headers)).raise_for_status()
        detail_data = resp2json(resp=resp); fid = detail_data["data"]["list"][0]["fid"]; share_fid_token = detail_data["data"]["list"][0]["share_fid_token"]; download_result['detail_data'] = detail_data; time.sleep(0.1)
        # clouddrive/file/info/path_list
        params = {"pr": "ucpro", "fr": "pc", "uc_param_str": "", "__dt": "1266", "__t": f"{int(time.time() * 1000)}"}
        (resp := session.post('https://drive-pc.quark.cn/1/clouddrive/file/info/path_list', params=params, json={"file_path": ["/来自：分享"]}, cookies=cookies, headers=headers)).raise_for_status()
        path_list_data = resp2json(resp=resp); to_pdir_fid = path_list_data["data"][0]["fid"]; download_result['path_list_data'] = path_list_data; time.sleep(0.1)
